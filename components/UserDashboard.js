    ...
    return (
        <div className='w-full max-w-[65ch] text-xs sm:text-sm mx-auto flex flex-col flex-1 gap-3 sm:gap-5'>
            <div className='flex items-stretch'>
                <input type='text' placeholder="Please Enter Remainders" value={todo} onChange={(e) => setTodo(e.target.value)} className="outline-none p-3 text-base sm:text-lg text-slate-900 flex-1" />
                <button onClick={handleAddTodo} className='w-fit px-3 sm:px-3 py-2 sm:py-3 bg-amber-400 text-black font-medium text-base duration-300 hover:opacity-40'>ADD REMAINDER</button>
            </div>
